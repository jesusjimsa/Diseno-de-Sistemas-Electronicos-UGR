...
{
    uint16 count;
    uint8 buffer[1];//[USBUART_BUFFER_SIZE];
    uint8 cmpvalue;
        ...
        if (0u != USBUART_GetConfiguration())
        {
            ...
            if (0u != USBUART_DataIsReady())
            {
                ...
                if (0u != count)
                {
                    ...
                    if(buffer[0] == 'q'){
                        LCD_Position(0u, 0u);
                        LCD_PrintString("Apagado");
                        PWM_Stop();
                    }
                    else if(buffer[0] == 'w'){
                        LCD_Position(0u, 0u);
                        LCD_PrintString("Encendido");
                        PWM_Start();
                    }
                    else if(buffer[0] == 'e'){
                        LCD_Position(0u, 0u);
                        LCD_PrintString("Duty +10");
                        cmpvalue = (cmpvalue + 10)%100;
                        PWM_WriteCompare(cmpvalue);
                    }
                    else if(buffer[0] == 'r'){
                        LCD_Position(0u, 0u);
                        LCD_PrintString("Duty -10");
